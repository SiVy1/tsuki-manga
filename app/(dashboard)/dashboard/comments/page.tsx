...
export default async function DashboardCommentsPage({ searchParams }: PageProps) {
  ...
  async function hideComment(formData: FormData) {
    ...
    redirect(`/dashboard/comments?notice=${encodeURIComponent("Comment hidden.")}`);
  }
  ...
  async function deleteComment(formData: FormData) {
    ...
    redirect(`/dashboard/comments?notice=${encodeURIComponent("Comment removed.")}`);
  }
  ...
  async function resolveReport(formData: FormData) {
    ...
    redirect(`/dashboard/comments?notice=${encodeURIComponent("Report resolved.")}`);
  }
  ...
  async function rejectReport(formData: FormData) {
    ...
    redirect(`/dashboard/comments?notice=${encodeURIComponent("Report rejected.")}`);
  }

  return (
    <div className="space-y-6">
      <section className="space-y-2">
        <p className="text-xs uppercase tracking-[0.24em] text-muted">Comments</p>
        <h1 className="font-serif text-4xl">Open reports</h1>
        <p className="max-w-2xl text-sm text-muted">
          Review reported chapter comments without turning moderation into a separate control
          center.
        </p>
      </section>
      ...
      <section className="space-y-4">
        {reports.length ? (
          reports.map((entry) => (
            <article key={entry.id} className="panel space-y-5 p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2.5 text-xs uppercase tracking-[0.16em] text-muted">
                    ...
                    <span>·</span>
                    <span>{entry.reportCount} report{entry.reportCount === 1 ? "" : "s"}</span>
                    <span>·</span>
                  ...
                  <p className="text-sm text-muted">
                    Reported comment by {entry.comment.authorName}. Posted {entry.comment.createdAtLabel}.
                  </p>
                </div>
              ...
              <div className="space-y-3 rounded-[1.4rem] bg-[var(--surface-muted)] px-5 py-4">
                <p className="text-xs uppercase tracking-[0.16em] text-muted">Comment</p>
                <p className="max-w-3xl whitespace-pre-wrap text-sm leading-7 text-foreground">
                  {entry.comment.excerpt}
                </p>
                {entry.latestDetails ? (
                  <p className="text-sm text-muted">Moderator note from report: {entry.latestDetails}</p>
                ) : null}
              </div>
          ...
          <div className="panel space-y-3 p-6">
            ...
            <p className="text-sm text-muted">
              The moderation queue is clear for now.
            </p>
          </div>
        )}
      </section>
